...
> 版本：0.5（机器 SemVer：0.5.0）| 最后更新：2026-08-21
...
| `folder_name_tag_settings.py` | 文件夹名称标签全局黑名单的校验、版本与原子持久化 |
...
- 设备配对（`pairing.py`）
- 查询图片上传（`uploads.py`）
- 推荐批次及 shown/action 事件（标准 `/api/v1/recommendations`、手表 `/api/v1/watch/recommendations`）
...
- 手表将响应中的媒体路径统一挂到用户当前填写的服务器 IPv4/端口，确保缩略图和原图继续通过同一个 FRP 公网入口访问。它并行预载五张缩略图，任意两张成功后即可显示整批槽位，尾部继续加载；加载下一批期间必须保留当前五张，并在当前批 settle 后至多静默准备一批。点图先保留已缓存缩略图，只请求所选图片的 `/original`，并暂停低优先级预热；失败可在原位置重试，不得为查看单图下载其他原图。
...
- 入口位于“批量标签”页的“从文件夹名生成”；默认处理整个当前图库，同时保留当前文件夹（含子文件夹）范围。首次整库迁移默认强制重跑，不导入旧 `quick_tag_state.txt`。
- 操作必须先执行只读预览，再明确确认提交后台任务。预览返回规则版本、待更新图片、变化文件夹、黑名单/历史脏标签/重复项移除数及最多 100 个文件夹样例；应用必须携带预览规则版本，规则已变化时拒绝执行并要求重新预览。
- “清理并重建”依次清理手工 `tags` 与继承 `inherited_tags` 中的历史脏标签、黑名单前缀和大小写重复项，再完整重建独立的 `folder_tags` 来源。模型 `accepted_auto_tags` 与系统元数据原值不得修改；最终有效标签仅在展示/检索层按 NFKC、trim、casefold 跨来源去重并保留首个显示拼写。
- 高级模式包括“仅清理旧标签”和“仅标记已处理”；不提供整次撤销。任务状态写入图库 SQLite：同一规则版本下成功、无标签及仅标记文件夹可跳过，失败文件夹继续重试；文件夹重命名或规则版本变化自动按新键重跑。
- 全局黑名单位于“设置 → 标签规则”，单独原子保存到 `folder-name-tagging.json`，上限 256 条。规则加载时 NFKC、trim、大小写去重；普通规则对标签不区分大小写做前缀匹配，含空格规则只匹配清洗后的完整文件夹名。`V` 的宽前缀语义必须在 UI 明示。
- 标签来自直接包含图片的文件夹；清洗数量、容量、括号元数据及尾部扩展名后，纯 ASCII 或无有效标签时只回退一级父目录，不得越过图库根目录。未来文件夹继承候选同样应用当前黑名单，但不得回写或删除候选图片的模型标签。
- 应用任务按 128 张图片分页读取状态，复用现有图片向量，通过 Collection 写协调器同步更新 Zvec 与 SQLite；不调用模型、不重新生成图片向量，支持安全取消。只写一条聚合操作日志并保留有界失败明细，不为每张成功图片写日志。
- 后续正常索引使用同一文件夹规则生成 `folder_tags`。后端命令为 `folder_name_tag_estimate`（不进入任务历史）和 `folder_name_tag_apply`（进入任务历史并报告进度与失败项）；设置端点为 `GET/PUT /v1/folder-name-tags/settings`。
...
- **工作流程**：watchdog 后台监听已索引 root 目录 → 文件变化事件带单调 `event_sequence` 持久化到 `fs_change_queue` → 5 秒防抖等待无新事件 → 自动提交正式任务 `auto_index_and_auto_tag`。任务启动时固定本轮序列截止点，每批最多领取 200 个事件并调用 `index_and_auto_tag_incremental`；本轮期间到达的新事件留给下一轮
- **授权语义**：用户为图库启用 `auto_index_enabled` 即持续授权该图库的自动增量索引与智能标注流水线；watcher 提交内部任务时必须携带外部处理确认。手动智能标注任务仍须逐次明确确认
- **任务可见性**：每个稳定变更周期只创建一个正式任务，进入任务历史并持续更新处理数/总数；空闲 watcher 只在设置开关中体现，不创建常驻任务。取消只终止当前周期、释放已领取事件并保持监听启用，积压由后续周期继续处理
...
- **操作日志**：每轮完成、部分完成、失败或取消后写一条聚合日志，至少包含文件变化总数、created/modified/deleted、索引新增/更新/删除、自动标注成功/失败、批次数和剩余积压；成功项不逐图片写日志，失败图片明细最多保留 200 条
- **自反馈防护**：Windows watcher 必须排除 `FILE_NOTIFY_CHANGE_LAST_ACCESS`，避免索引读取图片自身再次产生修改事件；真实写入产生的 created/modified/deleted 仍正常入队
...
- watcher 的 `auto_index_and_auto_tag` 是正式可取消任务；任务筛选和详情使用“自动增量索引与标注”名称。完成后任务历史与聚合操作日志必须引用同一 `job_id`。
...
- **安卓端**：Gradle 构建 .apk
- **发布版本**：机器版本 `0.5.0`，公开展示版本 `0.5`，稳定标签 `v0.5.0`
- **Windows 版本资源**：发布载荷内三个可执行文件的产品名均为 `YaoLens`，文件版本与产品版本均展示为 `0.5.0`，固定数值版本为 `0.5.0.0`
- **发布矩阵**：仅发布 YaoLens Windows、Android 及对应校验、验证报告和策略元数据
- **公开附件**：`YaoLens-0.5-win-x64-portable.zip`、`YaoLens-0.5-win-x64-setup.exe`、`YaoLens-0.5-android.apk`
