"""Provider-neutral visual-tagging domain, validation, and parsing."""
...
class VisionTaggingError(ModelProviderError):
...
class VisionInputError(VisionTaggingError, ValueError):
    ...
    def __init__(self, message: str) -> None:
        super().__init__(message, category="input")
...
class VisionTransportError(VisionTaggingError):
    """Raised when a provider cannot return a usable HTTP response."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            message,
            category=category or _http_transport_category(status_code),
            status_code=status_code,
            code=code,
            attempts=attempts,
        )
...
def _http_transport_category(status_code: int | None) -> ProviderErrorCategory:
