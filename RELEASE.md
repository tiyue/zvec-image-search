...
本次使用机器版本 `0.5.0`、公开展示版本 `0.5` 和稳定标签 `v0.5.0`。

## 本版更新

### Qwen 单图编辑

- Windows 新增独立「图片编辑」工作区，支持文件选择、拖放、剪贴板粘贴和搜索结果右键导入；每张图片建立独立本地草稿，只有用户明确点击生成后才上传。
- 支持 Qwen Image 3.0、2.0、Image Edit Max、Plus 及固定快照模型，并按模型约束校验尺寸、比例、反向提示词、随机种子和提示词智能改写。
- 原图保持只读；生成结果下载并校验为 PNG 后原子写入用户目录。失败或取消会保留原图、指令和参数，允许手工重试。
- 修复数值随机种子提交以及带 MPF/MPO 辅助帧 JPEG 的导入兼容性。

### 文件夹名称标签全库重建

- 「批量标签 → 从文件夹名生成」支持先预览、再对当前文件夹或整个图库执行清理重建；首次全库迁移默认重新处理全部已索引图片。
- 全局黑名单按规则进行 NFKC 规范化和不区分大小写的前缀匹配；清理只作用于手工与继承标签，不修改模型标签或系统元数据。
- 文件夹来源标签独立保存，最终展示和检索跨来源按规范化文本去重并保留首个显示拼写；任务支持取消、断点状态和有界失败明细。

### 自动监听任务历史与操作日志

- watcher 每个增量批次现在以「自动索引与智能标注」正式任务进入任务历史，报告新增、修改、删除、处理及失败数量。
- 每批先冻结稳定事件序列边界；完整索引与标注成功后才确认事件，失败、取消或确认异常会恢复待处理状态，避免静默丢图。
- 完成后只写聚合操作日志，不为每张成功图片制造日志洪泛；Windows 文件访问时间变化不再误触发重新索引。
...
- Windows WebView 与 Android 同步提供“推荐”页面；每批目标为 15 张，固定配额为技术质量 5、低曝光 6、随机发现 4，不再生成最近入库来源。
- 候选按 SHA-256 去重，同一图集最多 3 张、同一已确认角色最多 5 张；推荐不再读取或限制作者维度。
- 已展示历史按 `240 → 210 → 180 → 150 → 120 → 90 → 60 → 30 → 0` 逐级放宽。候选不足时只使用其他合格候选补位，仍不足则返回部分批次，不复制图片凑数。
...
- Windows 与 Android 共享显式喜欢/不喜欢偏好；推荐多样性和个性化均只复用现有索引向量，不临时调用模型。
...
GitHub Release 标题固定为 `YaoLens 0.5`，并从精确指向工作流提交的稳定标签 `v0.5.0` 发布。
