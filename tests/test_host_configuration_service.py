...
class HostConfigurationServiceTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_update_library_persists_auto_index_enabled(self) -> None:
        snapshot = self.service.create_initial(self.images)
        library_id = snapshot.configuration.default_library_id
        ...
        self.assertTrue(changed.configuration.by_id[library_id].auto_index_enabled)
        reloaded = DesktopConfigurationService(
            self.config_path,
            config_home=self.config_home,
        ).load()
        assert reloaded is not None
        self.assertTrue(reloaded.configuration.by_id[library_id].auto_index_enabled)
