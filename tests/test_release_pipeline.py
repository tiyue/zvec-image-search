...
class ReleaseRequestPolicyTest(unittest.TestCase):
    def test_branch_release_can_explicitly_waive_search_certification(self) -> None:
        ...
        self.assertEqual(outputs["public_version"], "0.2")
...
class ReleaseAssemblyTest(unittest.TestCase):
    def test_assembles_only_webview_and_android_products(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            webview, android = _artifact_directories(root, "0.2.0")
            ...
            self.assertEqual(outputs["release_title"], "YaoLens 0.2")
            ...
            self.assertEqual(policy["public_version"], "0.2")
            ...
            self.assertIn("YaoLens-0.2-win-x64-portable.zip", names)
            self.assertIn("YaoLens-0.2-win-x64-setup.exe", names)
            self.assertIn("YaoLens-0.2-android.apk", names)
...
class ReleaseWorkflowContractTest(unittest.TestCase):
    ...
    def test_public_version_and_android_version_are_synchronized(self) -> None:
        root = repository_root()
        self.assertEqual(read_project_version(root), "0.2.0")
        self.assertEqual(public_version(read_project_version(root)), "0.2")
        android = (root / "android/app/build.gradle.kts").read_text(encoding="utf-8")
        self.assertIn("versionCode = 500008", android)
        self.assertIn('versionName = "0.2.0"', android)
        frontend = json.loads(
            (root / "frontend/package.json").read_text(encoding="utf-8")
        )
        self.assertEqual(frontend["version"], "0.2.0")
    ...
    def test_public_release_documents_use_stable_yaolens_names(self) -> None:
        ...
        self.assertIn("版本：0.2（机器 SemVer：0.2.0）", spec)
